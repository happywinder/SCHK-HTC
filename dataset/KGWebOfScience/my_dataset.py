...
base_path = "dataset"
data_path = os.path.join(base_path, "KGWebOfScience")

low_res_path = os.path.join(data_path, "low-res")
few_shot_path = os.path.join(base_path, "few-shot", "kGWebOfScience")
...
def process_data(path):
    ...
    with open(path, 'r') as fp:
        for line in list(fp):
            ...
            datas.append([data['doc_token'], label1.index(data['doc_label'][1]),[item['linked_id'] for item in data['linked_entities']]])
...
def few_shot_sample(dataset, shot=5, seed=171, type="train"):
    ...
    for idx, (text,label,entities) in enumerate(train):
