...
base_path = "dataset"
data_path = os.path.join(base_path, "WebOfScienceReason")

low_res_path = os.path.join(data_path, "low-res")
few_shot_path = os.path.join(base_path, "few-shot", "WebOfScienceReason")
...
def get_train_dev_test(mode=None):
    ...
    wos_dataset = dict()
    for p in mode:
        wos_dataset[p] = process_data(os.path.join(data_path, f"wos_{p}.json"),p)
    return wos_dataset


def process_data(path,mode='train'):
    ...
    with open(path, 'r') as fp:
        for line in list(fp):
            ...
            if mode=='train':
                datas.append([data['doc_token'], label1.index(data['doc_label'][1]),data['1_level'],data['2_level']])
                continue
...
def few_shot_sample(dataset, shot=5, seed=171, type="train"):
    ...
    for idx, (text,label,level_1,level_2) in enumerate(train):
        examples.append(InputExample(guid=str(idx), text_a="", label=label))
...
def sub_dataset(shot=16, seed=171, ratio=-1, ratio_flag=0):
