...
def main():
    ...
    parser.add_argument("--knowledge_emb_path",type=str,default="KG/deepwalk.pt")
    parser.add_argument("--knowledge_dir",type=str,default="KG/final_enriched_kg/entity2id.txt")
    parser.add_argument("--multi_mask", type=int, default=1)
    parser.add_argument("--description",default="dataset/KGWebOfScience/wos_label_description_10.json",type=str)
    ...
    parser.add_argument("--KG",default=0,type=int)
    ...
    parser.add_argument("--device", default=8, type=int)
    parser.add_argument("--origin",default=0,type=int)
    ...
    if args.KG:
        train_entities=processor.train_entities
        dev_entities=processor.dev_entities
        test_entities=processor.test_entities
    ...
    for i in range(args.depth):
        text_mask.append(f'{i + 1} level: {{"mask"}} rather than :{{"mask"}}')
    ...
    ## Loading dataset
    train_dataloader = SinglePathPromptDataLoader(dataset=dataset['train'], 
                                                  template=mytemplate, 
                                                  tokenizer=tokenizer,
    ...
    if args.dataset == "wos":
        ...
        full_name = "DBPedia"
    elif args.dataset =="kgwos":
        full_name = "KGWebOfScience"
    else:
    ...
    if args.dataset != "dbp" and os.path.exists(dev_path):
        ...
        validation_dataloader = SinglePathPromptDataLoader(dataset=dataset["dev"], 
                                                           template=mytemplate,
                                                           tokenizer=tokenizer,
                                                           tokenizer_wrapper_class=WrapperClass,
                                                           max_seq_length=max_seq_l,
    ...
    if not os.path.exists(test_path):
        test_dataloader = SinglePathPromptDataLoader(dataset=dataset["test"], 
                                                     template=mytemplate, 
                                                     tokenizer=tokenizer,
                                                     tokenizer_wrapper_class=WrapperClass, 
                                                     max_seq_length=max_seq_l,
    ...
    this_run_unicode = f"{args.dataset}-seed{args.seed}-shot{args.shot}-lr{args.lr}-lr2{args.lr2}-batch_size{args.batch_size}-multi_mask{args.multi_mask}-use_new_ct{args.use_new_ct}-cs_mode{args.cs_mode}-ctl{args.contrastive_logits}" \
                       f"-contrastive_alpha{contrastive_alpha}-shuffle{args.shuffle}-constraint_loss{args.constraint_loss}-multi_verb{args.multi_verb}-origin{args.origin}" \
    ...
    if args.training:
        for epoch in range(args.max_epochs):
            ...
            idx = 0
            if args.KG:
                for batch,entity in tqdm(zip(train_dataloader,train_entities)):
            ...
            if args.KG:
                scores = prompt_model.evaluate(validation_dataloader, processor, desc="Valid",
                                            mode=args.eval_mode,entities=dev_entities)
            else:
                scores = prompt_model.evaluate(validation_dataloader, processor, desc="Valid",
                                            mode=args.eval_mode)
            early_stop_count += 1
    ...
    if args.eval_full:
        ...
        for k in best_keys:
            ...
            scores = prompt_model.evaluate(test_dataloader, processor, desc="test", mode=args.eval_mode,entities=test_entities,
                                           args=args)
            tmp_str = ''
        ...
        # for best macro
        # prompt_model.load_state_dict(torch.load(f"ckpts/wos-seed137-shot16-lr5e-05-lr20.0001-batch_size5-multi_mask1-use_new_ct1-cs_mode0-ctl1-contrastive_alpha0.99-shuffleFalse-constraint_loss1-multi_verb1-contrastive_level1--use_dropout_sim1-length2128-macro.ckpt"))
        ...
        scores = prompt_model.evaluate(test_dataloader, processor, desc="test",entities=test_entities, mode=args.eval_mode)
        macro_f1_1 = scores['macro_f1']
        micro_f1_1 = scores['micro_f1']
        acc_1 = scores['acc']
        prompt_model.get_predict_result()
        print_info('finally best macro {} {} micro {} acc {}'.format(best_score_macro_epoch, macro_f1_1, micro_f1_1, acc_1))

        # for best micro
        # prompt_model.load_state_dict(torch.load(f"ckpts/wos-seed137-shot16-lr5e-05-lr20.0001-batch_size5-multi_mask1-use_new_ct1-cs_mode0-ctl1-contrastive_alpha0.99-shuffleFalse-constraint_loss1-multi_verb1-contrastive_level1--use_dropout_sim1-length2128-micro.ckpt"))
