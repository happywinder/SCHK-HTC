...
from models.loss import constraint_multi_depth_loss_func, flat_contrastive_loss_func,constraint_multi_depth_loss_func_inverse
import torch.nn.functional as F
import json
from .loss import ZLPRLoss
...
class HierVerbPromptForClassification(PromptForClassification):
    def __init__(self,
        ...
        self.neg_linear_list=[]
        self.label_emb=[]
        self.args = args
        if self.args.KG:
            self.node2vec=torch.load("KG/deepwalk.pt").to("cuda")
            self.linkedid2node=self.load_linkedid("KG/final_enriched_kg/entity2id.txt")
            self.KG_model=BertModel.from_pretrained("bert-base-uncased")
            self.KG_tokenizer=BertTokenizer.from_pretrained("bert-base-uncased")
        ...
        self.build_label_emb()
        self.build_neg_linear_list()
        ...
        self.count3=0
        self.count4=0
        self.false_sibling_count=0
        if self.args.KG:
            self.build_knowledge_emb()


    def build_knowledge_emb(self):
        emb=torch.load(self.args.knowledge_emb_path)
        with open(self.args.knowledge_dir,'r') as f:
            lines=f.readlines()
        self.knowlege_emb={}
        for i in range(len(lines))[1:]:
            self.knowlege_emb[lines[i].split('\t')[0]]=emb[i-1]
    
    def load_linkedid(self,file_path):
        linked2node={}
        with open(file_path,"r") as f:
            data=f.readlines()
        for item in data[1:]:
            linked_id,node_id=item.strip().split("\t")
            linked2node[linked_id]=int(node_id)
        return linked2node

    def build_label_emb(self):
        with open(self.args.description,"r")as f:
            desc_dict=json.load(f)
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = BertModel.from_pretrained('bert-base-uncased')
        model.eval()
        model = model.to('cuda' if torch.cuda.is_available() else 'cpu')
        for i in range(self.verbLength):
            embeddings=[]
            for label in self.processor.label_list[i]:
                text = desc_dict[label]
                ...
                with torch.no_grad():
                    ...
                    avg_hidden = sum_hidden / mask.sum(dim=1)  # shape: [1, hidden_dim]

                    embeddings.append(avg_hidden.squeeze(0))  # 去掉 batch 维度
            embedding_matrix = torch.stack(embeddings) 
            self.label_emb.append(embedding_matrix) 
                
    def build_neg_linear_list(self):
        for i in range(self.verbLength):
            self.neg_linear_list.append(nn.Sequential(
                            nn.Linear(768, 768),       
                            nn.GELU(),                
                            nn.LayerNorm(768),        
                            nn.Dropout(p=0.1)          
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        labels = batch['label']
        if self.args.KG:
            kg_text=batch['entity']
            kg_t=[item['linked_name'] for item in kg_text]
            # g_input="".join(kg_t)
            KG_encode=self.KG_tokenizer(
                kg_t,
                truncation=True,
                max_length=512
            )
            KG_template=f"the first layer's knowledge is [MASK] the second layer's knowledge is [MASK]"
            template_input_ids=self.KG_tokenizer(KG_template,return_tensors="pt")['input_ids']
            KG_input=torch.zeros((512),dtype=torch.long).to(self.device)
            KG_input[0:template_input_ids.shape[-1]-1]=template_input_ids[0][0:-1]
            start=template_input_ids.shape[-1]
            id_text={}
            for i in range(len(KG_encode['input_ids'])):
                
                len1=len(KG_encode['input_ids'][i])-2
                if start+len1<511:
                    KG_input[start:start+len1]=torch.tensor(KG_encode['input_ids'][i][1:-1],dtype=torch.long).to(self.device)
                    id_text[kg_text[i]['linked_id']]=(start,start+len1)
                    start= start+len1
                    
            KG_input[start]=102
            KG_token_type_ids=torch.tensor([0]*KG_input.shape[0], dtype=torch.long).to(self.device)
            att_pad = [1]*(start+1)
            att_pad.extend([0]*(KG_input.shape[0]-start-1))
            KG_attention_mask=torch.tensor(att_pad, dtype=torch.long).unsqueeze(0).to(self.device)
            input_embeddings=self.KG_model.embeddings(
                input_ids=KG_input.unsqueeze(0),
                token_type_ids=KG_token_type_ids.unsqueeze(0),
            )# [batch,sequence,hidden]
            for key,value in id_text.items():
                linked_id=key
                inject_start,inject_end=value
                input_embeddings[:,inject_start:inject_end,:]+=self.node2vec[self.linkedid2node[linked_id]].unsqueeze(0) # 
            knowledge_aware=self.KG_model(
                inputs_embeds=input_embeddings,
                attention_mask=KG_attention_mask
            )
            mask_token_id = self.tokenizer.mask_token_id
            masked_indices=torch.tensor([i for i,token_id in enumerate(template_input_ids.squeeze(0)) if token_id==mask_token_id],dtype=torch.long,device=self.KG_model.device)
            knowledges_logits=torch.index_select(knowledge_aware.last_hidden_state,1,masked_indices)

        logits = []
        alpha=0
        for idx in range(self.args.depth*self.verbLength)[0::outputs_at_mask.shape[-2]//self.args.depth]:
            if self.args.KG:
                label_words_logtis = self.__getattr__(f"verbalizer{idx // self.args.depth}").process_outputs(outputs_at_mask[:, idx, :]+alpha*knowledges_logits[:,idx//self.args.depth,:],batch=batch)
            else:
                label_words_logtis = self.__getattr__(f"verbalizer{idx // self.args.depth}").process_outputs(outputs_at_mask[:, idx, :],batch=batch)
        ...
        if self.training:
            ...
            for idx, cur_depth_label in enumerate(hier_labels):
                ...
                co_level_topk_indices_list = []
                sibling_topk_indices_list=[]
                for i in range(cur_depth_logits.size(0)):
                    item_logits = cur_depth_logits[i]
                    gt = cur_depth_label[i]
                    
                    logits_exclude_gt = item_logits.clone()
                    logits_exclude_gt[gt] = -float('inf')

                    topk = torch.topk(logits_exclude_gt, k=2, dim=-1)
                    co_level_topk_indices_list.append(topk.indices)

                if args.origin!=1:
                    if idx>0:
                        for k in range(cur_depth_logits.size(0)):

                            label1_to_label0_mapping=self.processor.label1_to_label0_mapping
                            label0_to_label1_mapping=self.processor.label0_to_label1_mapping
                            sibling_topk_indices_list.append(self.get_topk_siblings_excluding_gt_v2(cur_depth_logits[i],cur_depth_label[i],label1_to_label0_mapping  ,label0_to_label1_mapping))

                        sibling_topk_tensor=torch.tensor(sibling_topk_indices_list,dtype=torch.long).to(device=args.device)
                        for k in range(sibling_topk_tensor.shape[0]):
                            a=self.neg_linear_list[idx](outputs_at_mask[:, 2*idx+1, :])
                            for j in range(3): 
                                b=self.label_emb[idx][j,:]
                                loss+=(1-F.cosine_similarity(a,b)/co_level_topk_indices_tensor.shape[0]).sum(dim=-1)
                    co_level_topk_indices_tensor = torch.stack(co_level_topk_indices_list).to(device=args.device) 
                    for i in range(co_level_topk_indices_tensor.shape[0]):
                        a=self.neg_linear_list[idx](outputs_at_mask[:, 2*idx+1, :])
                        for j in range(2):
                            b=self.label_emb[idx][j,:]
                            loss+=(1-F.cosine_similarity(a,b)/co_level_topk_indices_tensor.shape[0]).sum(dim=-1)
    ...
    def get_topk_siblings_excluding_gt_v2(self,logits_l1: torch.Tensor,
                                      gt_l1: torch.Tensor,
                                      label1_to_label0_mapping: dict,
                                      label0_to_label1_mapping: dict,
                                      top_k: int = 3):
            """
            获取每个样本中，除了 ground truth 外，同一个父节点下的 top-k label1 logits 下标
            返回：List[List[int]]，每个子列表是 label1 的 global 索引
            """
            result_idx = []

            gt_label1 = gt_l1.item()
            if gt_label1 not in label1_to_label0_mapping:
                result_idx.append([])
                return
            
            parent_label0 = label1_to_label0_mapping[gt_label1]
            sibling_candidates = label0_to_label1_mapping[parent_label0]

            candidate_labels = [l for l in sibling_candidates if l != gt_label1]

            if len(candidate_labels) == 0:
                result_idx.append([])
                return

            logits_vals = logits_l1[candidate_labels]
            topk_vals, topk_indices = torch.topk(logits_vals, min(top_k, len(candidate_labels)))

            topk_label1_indices = [candidate_labels[idx] for idx in topk_indices.tolist()]
            

            return topk_label1_indices
    ...
    def evaluate(self, dataloader, processor, desc="Valid", mode=0, device="cuda:0", entities=None,args=None):
        self.eval()
        pred = []
        truth = []
        ...
        if self.args.KG:
            for step, (batch,entity) in enumerate(zip(pbar,entities)):
                batch = tuple(t.to(device) if isinstance(t, torch.Tensor) else t for t in batch)
                batch = {"input_ids": batch[0], "attention_mask": batch[1],
                            "label": batch[2], "loss_ids": batch[3],"entity":entity}
                logits = self(batch)
                ...
                for idx in range(depth - 2, -1, -1):
                    ...
                    hier_labels.insert(0, cur_depth_labels)# 只记录了叶子标签，通过hier_mapping映射
                ...
                for pred_, gold_ in zip(batch_preds, batch_golds):
                    pred_parent, pred_child = pred_
                    gold_parent, gold_child = gold_
                    mapping = self.processor.label1_to_label0_mapping
                    if pred_parent == gold_parent and pred_child == gold_child:
                        self.count1+=1
                    elif pred_parent == gold_parent and pred_child != gold_child:
                        self.count2+=1
                    elif pred_parent != gold_parent and pred_child == gold_child:
                        self.count3+=1
                    else:
                        self.count4+=1
                    if mapping[pred_child]==mapping[gold_child]:
                        if pred_child==gold_child:
                            continue
                        self.false_sibling_count+=1
    ...
    def get_predict_result(self):
        print("全对",self.count1)
        print("父类对，子类错",self.count2)
        print("父类错，子类对",self.count3)
        print("父类错，子类错",self.count4)
        print("父类对，子类错在同级分类错误",self.false_sibling_count )
        self.count1=0
        self.count2=0
        self.count3=0
        self.count4=0
