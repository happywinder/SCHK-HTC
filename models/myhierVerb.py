...
class HierVerbPromptForClassification(PromptForClassification):
    def __init__(self,
        ...
        with open(os.path.join(data_dir,"seed_"+str(args.seed)+"-shot_"+str(args.shot)+".json")) as f:
            index=json.load(f)
        self.index=index
    ...
    def forward(self, batch) -> torch.Tensor:
        ...
        if self.training:
            ...
            if lm_loss is not None:
                if args.lm_alpha != -1:
                    loss = loss * args.lm_alpha + (1 - args.lm_alpha) * lm_loss # lm_alpha=0.999 
                else:
                    loss += lm_loss
            ...
            if constraint_loss is not None:
                if args.constraint_alpha != -1:
                    loss = loss * args.constraint_alpha + (1 - args.constraint_alpha) * constraint_loss # constraint_alpha 0.99
                else:
    ...
    def init_prove_embeddings(self):
        import json
        from collections import defaultdict

        # 初始化字典
        empty_list=[{} for _ in range(self.args.depth)]

        selected_data=[]

        with open("dataset/WebOfScienceReason/wos_train.json", 'r', encoding='utf-8') as f:
        ...
        for i in range(self.args.depth):
            for item in selected_data:
                label=item['doc_label'][i]
                text = item.get(str(i+1)+"_level", "").strip()
                if label in empty_list[i].keys():
                    empty_list[i][label].append(text)
                else:
                    empty_list[i][label]=[text]
        dict_list=empty_list
        label_dict = self.processor.label_list[1]
        for item in label_dict:
            if item not in dict_list[1]:
                print("-------------")
                print(f"{item}")
                print("-------------")
                dict_list[1][item]=[item]
        ...
        with torch.no_grad():
            for entry in tqdm(dict_list):
                processed_entry = {}
                for label, texts in entry.items():
                    cls_vectors = []
                    for text in texts:
                        inputs = tokenizer(text, return_tensors="pt", truncation=True, padding=True, max_length=512)
                        inputs = {k: v.to(device) for k, v in inputs.items()}
                        outputs = model(**inputs)
                        cls_vec = outputs.last_hidden_state.mean(dim=1)  # [CLS] token
                        cls_vectors.append(cls_vec.squeeze(0))
                    # 平均池化
                    mean_cls = torch.stack(cls_vectors, dim=0).mean(dim=0)
                    processed_entry[label] = mean_cls.cpu()
                processed_data.append(processed_entry)
        # print(processed_data)
        label_emb_list=[]
        for idx in range(self.args.depth):
            label_dict = self.processor.label_list[idx]
            label_dict = dict({id: value for id,(key,value) in enumerate(processed_data[idx].items())})
            label_emb=[]
            for i in range(len(label_dict)):
                label_emb.append(label_dict[i])
    ...
    def init_embeddings(self):
        ...
        for idx in range(self.args.depth):
            ...
            label_dict = {i: self.tokenizer.encode(v) for i, v in label_dict.items()} # {0: [101, 20116, 102], 1: [101, 2966, 102], 2: [101, 2942, 102], 3: [101, 14925, 2063, 102], 4: [101, 22419, 102], 5: [101, 11530, 102], 6: [101, 6825, 102]}
            label_emb = []
