...
def constraint_multi_depth_loss_func_inverse(logits, loss_func, hier_labels, processor, args, use_cuda=True, alpha=0.3,mode=0):
    if isinstance(logits, list):
        parent_logits = logits[0] # leaf logits
    elif isinstance(logits, torch.Tensor):
        parent_logits = logits[:, 0, :]
    contrastive_level = 0
    hier_mapping = processor.hier_mapping # [label0_to_label1_mapping, label1_to_label0_mapping]
    ...
    parent_logits = torch.softmax(parent_logits, dim=-1) # 
    hier_logits = []
    hier_logits.insert(0, parent_logits)
    batch_s = parent_logits.shape[0]
    constraint_loss = 0

    all_length = len(processor.all_labels) # label_num
    child2parent = processor.hier_mapping[-1][1]
    for depth_idx in range(1, depth):
        if isinstance(logits, list):
            ori_logits = logits[depth_idx]
        elif isinstance(logits, torch.Tensor):
            ori_logits = logits[:, depth_idx, :]
        ori_logits=torch.softmax(ori_logits,dim=-1)
        if args.multi_verb:
            cur_logits = torch.zeros(batch_s, len(processor.label_list[depth_idx])) # [10,134]

            for i in range(cur_logits.shape[-1]): # 134
                # sum
                parent_idx = child2parent[i]
                cur_logits[i,:] = (1 - alpha)*ori_logits[i]+alpha*hier_logits[-1][parent_idx]
...
def constraint_multi_depth_loss_func(logits, loss_func, hier_labels, processor, args, use_cuda=True, mode=0):
    if isinstance(logits, list):
        leaf_logits = logits[-1] # leaf logits
    elif isinstance(logits, torch.Tensor):
    ...
    loss_weight = [1 / 2 ** (i * contrastive_level) for i in range(depth - 1)] # 权重
    ...
    all_length = len(processor.all_labels) # label_num
    for depth_idx in range(depth - 2, -1, -1): # from down 2 top
        if isinstance(logits, list):
            ori_logits = logits[depth_idx]
        elif isinstance(logits, torch.Tensor):
            ori_logits = logits[:, depth_idx, :]
        ...
        if args.multi_verb:
            ...
            for i in range(cur_logits.shape[-1]): # 7
                # sum
...
class ZLPRLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        """
        logits: Tensor of shape [B, C] — 分类得分
        labels: Tensor of shape [B] — 每个样本的真实类别索引
        """
        B, C = logits.size()
        ...
        for i in range(B):
            pos_logit = logits[i, labels[i]]
            neg_mask = torch.ones(C, dtype=torch.bool, device=device)
            neg_mask[labels[i]] = False
            neg_logits = logits[i][neg_mask]  # 负类分数

            diff = neg_logits - pos_logit  # 正类减负类（越大越好）
